...
> **Note:** this post's code is shown but not executed. It depends on `maptools`,
> archived from CRAN in 2023 and no longer compilable against current R —
> it uses `Calloc`/`Free` C macros removed from R's toolchain. Re-running this
> post would need the mapping code rewritten against `sf` or an equivalent
> modern package. The code below is plain syntax-highlighted text, not a
> live code cell — this page has no R dependency at all, on Netlify or
> anywhere else.
...
**Figure 1**: Map showing the southern tip of the African continent. The top panel shows the typical sea surface temperature and surface currents on January 1st. The bottom panel likewise shows the typical surface air temperatures and winds on any given January 1st. (No longer rendered here — see the note at the top of this post.)
