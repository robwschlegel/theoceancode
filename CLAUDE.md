...
- `content/post/mapping_with_ggplot2.qmd` sources `static/func/scale.bar.func.R`, a ggplot2 scale-bar/north-arrow helper. It originally depended on `maptools::gcDestination()` for the great-circle destination-point math; `maptools` was archived from CRAN in 2023 and no longer compiles (`Calloc`/`Free` C macros removed from R's toolchain). Rather than keep working around that, `gcDestination()` is now reimplemented directly in `scale.bar.func.R` (~15 lines, plain spherical trigonometry, verified against known geodesic distances) — the post has no `maptools` dependency at all anymore and executes normally like every other post.
